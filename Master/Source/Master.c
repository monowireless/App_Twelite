	...
	case E_STATE_RUNNING:
		...
		if (eEvent == E_EVENT_APP_TICK_A && (sAppData.u32CtTimer0 & 1)) {
			...
			if (0
				...
				|| (IS_APPCONF_OPT_ON_PRESS_TRANSMIT()
						&& (!sAppData.sIOData_now.u32BtmBitmap)
						&& (u32TickCount_ms - u32TxLastDiClear < ON_PRESS_TRANSMIT_KEEP_TX_ms)) // ボタンが離されてから 1000ms 間
#endif
...
void vProcessEvCoreSlp(tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	...
	case E_STATE_RUNNING:
		...
		if (sAppData.u8IOFixState == 0x3) {
			...
			sAppData.sIOData_now.i16TxCbId = i16TransmitIoData(TRUE); // クイックで送信
...
#if 0
		// 始動メッセージ (待ち処理が必要なため vProcessEvCore??? の  START_UP メッセージで表示するようにした)
		vWait(64000); // 少し待つ。最低37500必要 (UART 初期化直後パリティありで文字出力するとハングアップする v1.0.3)
	...
	case E_AHI_DEVICE_SYSCTRL:
		if (sAppData.u8IOFixState) {
			...
			for (i = 0; i < 4; i++) {
				...
				if (u8ct == 0) {
					sAppData.sIOData_now.au8Input[i] = 0x71;
					bTransmit = TRUE;
				} else {
...
#if 0 // 低レイテンシモードのため、以下の判定処理を TICK_TIMER に移動する
			if (bBTM_GetState(&bmPorts, &bmChanged)) {
				if (bmChanged) {
					// 入力ポートの値が確定したか、変化があった
					for (i = 0; i < 4; i++) {
...
PUBLIC uint8 cbToCoNet_u8HwInt(uint32 u32DeviceId, uint32 u32ItemBitmap) {
	...
	case E_AHI_DEVICE_TIMER0:
		...
		if (bAHI_APRegulatorEnabled()) {
			int i;
			for(i = 0; i < 2; i++) {
				vAHI_DacOutput(/* E_AHI_AP_DAC_1 or 2 */ i, sAppData.sIOData_now.au16OutputDAC_val[i]);
			}
		}
#endif
		break;
...
static void vReceiveIoData(tsRxDataApp *pRx) {
	...
	if (bAHI_APRegulatorEnabled()) {
		...
		for (i = 0; i < 2; i++) {
			if (sAppData.sIOData_now.au16OutputPWMDuty[i] != 0xFFFF) {
				int j = sAppData.sIOData_now.au16OutputDAC[i] * 4096 / ADC_MAC_MV;
				if (j >= 4095) j = 4095;
				sAppData.sIOData_now.au16OutputDAC_val[i] = j;

				vAHI_DacOutput(/* E_AHI_AP_DAC_1 */ i, j);
			}
		}
	}
#endif
